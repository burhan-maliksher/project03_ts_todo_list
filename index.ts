...
import { add_todo } from "./src/_Type.js";
import {titlevalidate} from "./src/_PromptValidator.js"


class _Todo {
  private _selected_menu!: string;
  private _add_todo!: add_todo[];
  private _view_todo_title!:string;
  ...
  private async AppTitle():Promise<void> {
    const title = chalkAnimation.neon(
      `______________________Welcome to M.B Todo List App______________________`
    );
    await titleTimer();
    title.stop();
    console.log(
  ...
  private async Home():Promise<void> {
    ...
    const check: string = this._selected_menu;
    switch (check) {
      case "ADD Todo":
        this.Add();
        break;
  ...
  private async MarkUnMark():Promise<void>{
    let view: add_todo[] = this._add_todo;
    ...
    const mark:string=markOption.check
  ...
  private async Back_to_home():Promise<void> {
    this.Home()
  }
